...
class Estimation:
    ...
    def Likelihood(self,parameter):
        ...
        BetaMatrix=cr.BetaMatrix(DistanceMatrix,np.delete(parameter,-1),self.method)
    ...
    def GammaPriorGeneralPosterior(self,parameter,i):
        from scipy.stats import gamma
        return gamma.pdf(parameter[i],0.001,scale=1/0.001)*self.Likelihood(np.array((parameter)))
    def GammaPosteriorPhi(self,parameter):
        '''
        Different phi for different distance decay function has different meaning
        how to choose the prior?
        for powerlaw, the phi(omega?) should be 0.5-2
        for gradient, the phi should be 5-100
        '''
        from scipy.stats import gamma
        return gamma.pdf(parameter[2],0.001,scale=1/0.001)*self.Likelihood(np.array((parameter)))
